...
async function extractTextFromPDF(pdfBase64: string): Promise<string> {
  ...
  // Also try to find any plain text in the PDF
  const plainTextRegex = /BT\s*([\s\S]*?)\s*ET/gi;
  while ((match = plainTextRegex.exec(text)) !== null) {
    extractedText += match[1] + ' ';
  }
  
  // If we couldn't extract much, fall back to raw text extraction
  if (extractedText.length < 100) {
    // Extract anything that looks like readable ASCII text
...
function extractCodesWithRegex(text: string): { code: string; date: string }[] {
  ...
  // Pattern 1: Code followed by text and date (MM-DD-YYYY or MM/DD/YYYY)
  ...
  // Pattern 2: Date followed by code
  ...
  // Pattern 3: Just 3-digit codes that look like IRS codes (100-999 range, common codes)
  const commonCodes = ['150', '290', '291', '300', '301', '420', '421', '424', '570', '571', 
...
serve(async (req) => {
  ...
  try {
    ...
    const severityOrder: Record<string, number> = { critical: 0, high: 1, medium: 2, low: 3, routine: 4, unknown: 5 };
    timeline.sort((a, b) => {
