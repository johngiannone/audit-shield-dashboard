...
Deno.test("enforceRateLimit allows up to maxRequests then blocks", async () => {
...
Deno.test("enforceRateLimit allows requests for different endpoints independently", async () => {
  const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);
  const testUserId = `test-user-${crypto.randomUUID()}`;
